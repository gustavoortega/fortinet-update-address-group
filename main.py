...
GROUP_NAME = os.getenv('GROUP_NAME', "DeployBot IPv4")  # Valor predeterminado si la variable de entorno no está definida
PREFIX_NAME_OBJ = os.getenv('PREFIX_NAME_OBJ', "DeployBot")
VERIFY_SSL = False
GCP_PROJECT = os.getenv('GCP_PROJECT', "fortinet-scripts")
SOURCE = os.getenv('SOURCE', { "type": "dns", "hostname": "servers.deploybot.com"})
...
def main(event, context):
    ...
    if SOURCE.get("type") == "dns":
        ips_dns = obtener_ips_dns(SOURCE.get("hostname"))
    print(ips_dns)
    ...
    for ip in ips_dns:
        ...
        if f"{PREFIX_NAME_OBJ}_{ip}" not in [obj["name"] for obj in address_objects if obj.get("subnet","")!=""]:
                                    #  existing_ips = [obj for obj in address_objects if obj.get("subnet","")!=""]
            print(f'IP dentro if: {ip}')
            crear_o_actualizar_address_object(ip, address_objects)

    # Actualiza el grupo de direcciones con las nuevas IPs
    actualizar_grupo_address(ips_dns, address_objects)
